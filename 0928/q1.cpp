...
int main(void)
{
   char *slide="■", key;
   int x=1, y=1;
   int h_slide_length, v_slide_length,length_step;
   printf("슬라이드바 표시\n\n");
   ...
   do
   {
    ...
    key=getch();
    move_arrow_key(key, &x, &y, h_slide_length, v_slide_length, length_step);
   }while(key!=27);
   return 0;
}
...
void draw_rectangle_h(int c, int r)
{
    int i, j;
    unsigned char a=0xa6;
    unsigned char b[7]; 
...
void draw_horizontal_slide(int x, int y, int length, char *s)
{
	...
	gotoxy(1, y);
	draw_rectangle_h(real_length+1, 1);
	gotoxy(x+1, y+1);
	printf("%s", s);
...
void move_arrow_key(char key, int *x1, int *y1, int x_b, int y_b,int step)
{
	...
	case 72:  //위쪽(상) 방향의 화살표 키 입력
		*y1=*y1-step;
		if (*y1<step)	*y1=1; //y좌표의 최소값
		break;
	case 75:  //왼쪽(좌) 방향의 화살표 키 입력
		*x1=*x1-step;
		if (*x1<step)	*x1=1; //x좌표의 최소값
		break;
